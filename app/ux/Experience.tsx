...
export default function Experience() {
  ...
  return (
    <section id="experience" className="relative bg-white py-16 sm:py-20 lg:py-24 border-b-2" style={{ borderColor: getAdaptiveBorderColor(isDark), boxShadow: getAdaptiveShadow(isDark) }}>
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <div className="mb-8 sm:mb-10 lg:mb-12">
          <h2 className="text-var-title sm:text-3xl lg:text-4xl font-extrabold text-slate-800">{t("experience.title")}</h2>
          <p className="mt-2 text-var-body sm:text-lg text-slate-600">{t("experience.subtitle")}</p>
        </div>
          ...
          {loading
            ? Array.from({ length: 3 }).map((_, idx) => (
                <li key={idx} className="mb-8 sm:mb-10 ms-4 sm:ms-6">
                  <span className="absolute -start-2 sm:-start-3 flex h-5 w-5 sm:h-6 sm:w-6 items-center justify-center rounded-full bg-accent ring-2 sm:ring-4 ring-white" />
                  <div className="rounded-xl sm:rounded-2xl bg-slate-50 border border-slate-200 p-4 sm:p-6">
                    <div className="animate-pulse space-y-2">
                      ...
                      <div className="h-3 w-32 sm:w-40 bg-slate-200 rounded" />
                    </div>
                  </div>
                </li>
              ))
            : items.map((r, idx) => (
                <AnimatedItem key={idx} delayMs={idx * 120}>
                  <span className="absolute -start-2 sm:-start-3 flex h-5 w-5 sm:h-6 sm:w-6 items-center justify-center rounded-full bg-accent ring-2 sm:ring-4 ring-white" />
                  <div className="rounded-xl sm:rounded-2xl bg-slate-50 border border-slate-200 p-4 sm:p-6">
                    <p className="text-var-caption sm:text-sm text-slate-500">{formatPeriodFr(r.period)}</p>
                    <h3 className="mt-1 text-var-caption sm:text-base text-slate-800 font-semibold">{r.title}</h3>
                    <p className="text-var-caption sm:text-base text-slate-600">{r.company}</p>
                    {r.summary && <p className="mt-2 text-var-caption sm:text-sm text-slate-600">{r.summary}</p>}
                  </div>
                </AnimatedItem>
              ))}
        </ol>
